...
private:
    ...
    // zdl add for android data transmit
    ...
    ros::Subscriber global_position_ref_subscriber;
...
public:
    ...
    //zdl add for android
    dji_sdk::TransparentTransmissionData data_from_mobile;
    bool start_mission, abort_mission, abort_landing;
...
private:
	...
	void global_position_subscriber_callback(dji_sdk::GlobalPosition global_position)
	{
        ...
        if (global_position_ref_seted == 0 && global_position.ts != 0 && global_position.latitude != 0 && global_position.health > 3) {
            this->global_position_ref = global_position;
    ...
    void data_from_mobile_subscriber_callback(dji_sdk::TransparentTransmissionData data_from_mobile_para)
    {
        this->data_from_mobile = data_from_mobile_para;
        for(int i=0; i<2; i++)
        {
            data[i] = data_from_mobile.data[i];
        }
        data[2] = '\0';
        if(data[0] == 's' && data[1] == 'm'){ start_mission = true; }
        if(data[0] == 'a' && data[1] == 'm'){ abort_mission = true; }
        if(data[0] == 'a' && data[1] == 'l'){ abort_landing = true; }
    }

public:
	DJIDrone(ros::NodeHandle& nh):
        start_mission(false), abort_mission(false), abort_landing(false),
		...
		waypoint_navigation_action_client(nh, "dji_sdk/waypoint_navigation_action", true)
	{
        ...
        data_received_from_remote_device_subscriber = nh.subscribe<dji_sdk::TransparentTransmissionData>("dji_sdk/data_received_from_remote_device",10, &DJIDrone::data_from_mobile_subscriber_callback,this);
...
//    bool send_to_mobile(std::vector<uint8_t> request)
//    {
//        dji_sdk::SendDataToRemoteDevice data_to_mobile;
//        data_to_mobile.request.data = request; // the field name of request is defined in the srv file
//        return send_data_to_remote_device_service.call(data_to_mobile) && data_to_mobile.response.result;
//    }

    void intToBytes(std::vector<uint8_t>& out, int corners[], uint8_t degree)
    {
        uint8_t tmp = 0;
        for (int i = 0; i < 8; i++)
        {
            tmp = (corners[i] >> 24) & 0xFF;
            out.push_back(tmp);
            tmp = (corners[i] >> 16) & 0xFF;
            out.push_back(tmp);
            tmp = (corners[i] >> 8) & 0xFF;
            out.push_back(tmp);
            tmp = (corners[i]) & 0xFF;
            out.push_back(tmp);
        }
